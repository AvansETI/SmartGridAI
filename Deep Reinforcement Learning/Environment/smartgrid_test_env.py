...
class SmartgridTestEnv(gym.Env):
    """
    Description:
    ...
    def __init__(self):
        ...
        # OpenAI spaces
        self.action_space      = spaces.Box(np.array([-self.battery.max_power_charge_w]), np.array([self.battery.max_power_discharge_w]), dtype=np.float16)
    ...
    def _is_done(self):
        return self.timestamp >= 31622400
    ...
    def _reward(self):
        ...
        state = {key: state[key] - self.prev_state.get(key, 0) for key in state.keys()}
        state['battery_capacity_wh'] = self.battery.capacity_wh
        return self.reward_function.calculate(state) # Call the reward function for the real rewards!
    ...
    def render(self, mode='human', close=False):
        print("smartgrid (" + str(self.timestamp) + "): " + str(self.grid_wh))
